...
# Config options for GSGAN
@click.option('--gaussian_num_pts',    help='the number of init gaussian.', metavar='FLOAT', type=click.IntRange(min=16), default=256, required=False, show_default=True)
@click.option('--start_pe',    help='Positional encoding on learnable constant', metavar='BOOL', type=bool, required=False, default=True)
@click.option('--center_dists',    help='coeff of center dist.', metavar='FLOAT', type=click.FloatRange(min=0), required=False, default=1.0)
@click.option('--prob_uniform',    help='probability to sample the pose from uniform distribution (for FFHQ)', metavar='FLOAT', type=click.FloatRange(min=0), required=False, default=0.0)
@click.option('--res_end',    help='end of resolution. refer to model architecture file', metavar='INT', type=click.IntRange(min=0), required=False, default=128)
...
def main(**kwargs):
    ...
    c.G_kwargs.class_name = 'training.gs_generator.GSGenerator'
    # c.D_kwargs.class_name = 'training.dual_discriminator.DualDiscriminator'
    c.D_kwargs.class_name = 'training.dual_discriminator.SingleDiscriminator' # TODO check
    ...
    rendering_options.update({
        'custom_options':{
            'scale_init': np.log((1 / np.sqrt(opts.gaussian_num_pts))),         # Refer to Appendix for details
            'scale_threshold': np.log((1 / np.sqrt(opts.gaussian_num_pts))),    # Refer to Appendix for details
            'scale_end': np.log(1 / c.training_set_kwargs.resolution),          # Refer to Appendix for details
            'scale_init_bg': -2,
            'scale_threshold_bg': -2,
            'scale_upsample_reg': 0,
            'xyz_output_scale': 0.1,            # Additional scale for coarsest xyz           
            'res_end': opts.res_end,            # Actual upsampling is performed by log2(res_end / 4) times
            'rgb_resolution': opts.res_end * 2, # maybe not needed
            ...
            'output_multiplier': 1,             # Multiplier for output points (not being used)
            'camera_noise_std': 0.0,            # noise for camera direction condition
            'use_dir_cond': True,               # use cam direction condition on color layer
        }
    })
    
    # Configuration about loss functions
    c.loss_kwargs.update({
        'loss_custom_options':{
            'knn_dists': 10.0,                  # coeff. of KNN distance
            'knn_num_ks': 64,                   # the number of KNN for calculating loss
            'center_dists': opts.center_dists,  # coeff. of center distance
            'pose_contrastive': 1.0,            # coeff. of pose contrastive loss
            'prob_uniform': opts.prob_uniform,  # 0.5 for FFHQ, 0.0 for AFHQ
            # FFHQ pose distribution
            'horizontal_type': 'gaussian',
            'horizontal_std': 16.610813365115508 / 180, # horizontal std of FFHQ
            'vertical_type': 'uniform',
            'vertical_std': 7.440431327985016 / 180 * 3, # 3x vertical std of FFHQ
            # 
            'contrastive_negpose_mult': 0,      # sample negative pose from predefined distribution (not being used)
            'disable_background': False,        # disable background generator
    ...
    c.loss_kwargs.dual_discrimination = False # discriminator has a single input image
    ...
    # Augmentation.
    if opts.aug != 'noaug':
        c.augment_kwargs = dnnlib.EasyDict(class_name='training.augment.AugmentPipe', xflip=0, rotate90=1, xint=1, scale=1, rotate=1, aniso=1, xfrac=1, brightness=1, contrast=1, lumaflip=1, hue=1, saturation=1)
        if opts.aug == 'ada':
