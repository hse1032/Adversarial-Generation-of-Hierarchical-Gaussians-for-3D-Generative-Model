...
def gen_interp_video(G, mp4: str, seeds, shuffle_seed=None, w_frames=60*4, kind='cubic', grid_dims=(1,1), num_keyframes=None, wraps=2, psi=1, truncation_cutoff=14, cfg='FFHQ', image_mode='image', gen_shapes=False, device=torch.device('cuda'), gs_params=None, **video_kwargs):
    grid_w = grid_dims[0]
    grid_h = grid_dims[1]
    ...
    _ = G.synthesis(ws[:1], c[:1], gs_params=gs_params) # warm up
        ...
        for yi in range(grid_h):
            for xi in range(grid_w):
                ...
                if entangle == 'conditioning':
                    ...
                    img = G.synthesis(ws=w_c, c=c_forward, noise_mode='const', gs_params=gs_params)[image_mode][0]
                elif entangle == 'camera':
                    
                    ret_dict = G.synthesis(ws=w.unsqueeze(0).to(torch.float32), c=c[0:1], noise_mode='const', gs_params=gs_params)
                    img = ret_dict[image_mode][0]
                    ...
                    w_c = G.mapping(z=zs[0:1], c=c[0:1], truncation_psi=psi, truncation_cutoff=truncation_cutoff)
                    img = G.synthesis(ws=w_c, c=c[0:1], noise_mode='const', gs_params=gs_params)[image_mode][0]
...
def generate_images(
    ...
    with dnnlib.util.open_url(network_pkl) as f:
        networks_pkl = legacy.load_network_pkl(f) # type: ignore
        
        if g_type == "G_ema":
            G_ema = networks_pkl["G_ema"].to(device)
            G = G_ema
        elif g_type == "G":
            G = networks_pkl["G"].to(device)
        else:
        ...
        if load_architecture:
            print("Load new architecture...", load_architecture)
            G_new = GSGenerator(**G.init_kwargs).to(device)
            
            state_dict = G.state_dict()
            state_dict.pop('dataset_label_std')
            G_new.load_state_dict(state_dict)
            G = G_new
    ...
    if interpolate:
        ...
        for seed in seeds:
            ...
            seeds_ = [seed]
            gen_interp_video(G=G, mp4=output, bitrate='10M', grid_dims=grid, num_keyframes=num_keyframes, w_frames=w_frames, seeds=seeds_, shuffle_seed=shuffle_seed, psi=truncation_psi, truncation_cutoff=truncation_cutoff, cfg=cfg, image_mode=image_mode, gs_params=gs_params)
