...
gpc_reg_prob=0.8
center_dists=10.0
prob_uniform=0.0 # only use for FFHQ
res_end=256
num_pts=256
nrr=512
...
expname=base
dataset=AFHQ512
dataset_path=../datasets/AFHQ_512.zip
outdir=../training-runs/${dataset}/${expname}
ngpus=8

# AFHQ512
python train.py --outdir=${outdir} --cfg=afhq --data=${dataset_path} \
  ...
  --center_dists=${center_dists} --prob_uniform=${prob_uniform} --res_end=${res_end} --aug=ada
