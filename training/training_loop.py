...
def training_loop(
    ...
    for name, module, opt_kwargs, reg_interval in [('G', G, G_opt_kwargs, G_reg_interval), ('D', D, D_opt_kwargs, D_reg_interval)]:
        if reg_interval is None:
            if name == "G":
                ...
                assert len(list(module.parameters())) == len(list(module.named_parameters())), "param: {} \t named_param:{}".format(len(list(module.parameters())), len(list(module.named_parameters())))

                params_groups = [] 
                params_groups.append({"params": model_params})
                for p, n in zip(gaussian_params, gaussian_params_names):
                    # TODO do we change the optimizer params for gaussians?
    ...
    while True:
        ...
        # TODO restore (save and restore) process
        # Save image snapshot.
        ...
        if (network_snapshot_ticks is not None) and (done or cur_tick % network_snapshot_ticks == 0):
            ...
            for name, module in [('G', G), ('D', D), ('G_ema', G_ema), ('augment_pipe', augment_pipe)]:
                if module is not None:
                    ...
                    # TODO custom deepcopy for densification?
