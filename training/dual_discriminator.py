...
class SingleDiscriminator(torch.nn.Module):
    def __init__(self,
        ...
        # TODO do not use conditional discriminator, use contrastive camera estimator instead
        c_dim_contrastive = c_dim
        c_dim = 0
        ...
        # Pose encoders
        self.aux_branch_feat = torch.nn.Sequential(
            FullyConnectedLayer(out_channels * 16, out_channels, activation='lrelu'),
            FullyConnectedLayer(out_channels, 64, activation='linear')
        )
        self.aux_branch_camera = torch.nn.Sequential(
            FullyConnectedLayer(c_dim_contrastive, out_channels, activation='lrelu'),
            FullyConnectedLayer(out_channels, 64, activation='linear')
        )
    ...
    def forward(self, img, c, update_emas=False, **block_kwargs):
        ...
        # Pose encoders
        feat_contrastive = self.aux_branch_feat(x_pose.flatten(1))
        c_contrastive = self.aux_branch_camera(c)
        
        aux_out = {
            'feat_contrastive': feat_contrastive,
            'c_contrastive': c_contrastive,
        }
...
@persistence.persistent_class
class SingleDiscriminator_contrastive(torch.nn.Module):
    def __init__(self,
        c_dim,                          # Conditioning label (C) dimensionality.
        img_resolution,                 # Input resolution.
        img_channels,                   # Number of input color channels.
...
#         # TODO custom
#         self.aux_branch_feat = torch.nn.Sequential(
#             FullyConnectedLayer(out_channels * 16, 256, activation='lrelu'),
#             FullyConnectedLayer(256, 64, activation='linear')
#         )
#         self.aux_branch_camera = torch.nn.Sequential(
#             FullyConnectedLayer(c_dim_contrastive, 256, activation='lrelu'),
#             FullyConnectedLayer(256, 64, activation='linear')
#         )
...
#         # TODO custom
#         feat_contrastive = self.aux_branch_feat(x.flatten(1))
#         c_contrastive = self.aux_branch_camera(c)
#         aux_out = {
#             'feat_contrastive': feat_contrastive,
#             'c_contrastive': c_contrastive,
...
@persistence.persistent_class
class SingleDiscriminator_cond(torch.nn.Module):
    def __init__(self,
        c_dim,                          # Conditioning label (C) dimensionality.
        img_resolution,                 # Input resolution.
        img_channels,                   # Number of input color channels.
        ...
        self.aux_branch_camera = torch.nn.Sequential(
            FullyConnectedLayer(c_dim, 256, activation='lrelu'),
            FullyConnectedLayer(256, 64, activation='linear')
        )
